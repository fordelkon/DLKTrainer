...
from .trainer_config import VQVAEnTrainerConfig
...
class VQVAEnTrainer(BaseTrainer):
    def __init__(self, model: nn.Module, dataset: Dataset, cfg: VQVAEnTrainerConfig):
        ...
        self.latent_weight = cfg.latent_weight
    ...
    def train(self):
        ...
        if self.amp:
            self.amp = check_amp(self.decoder)
        ...
        for epoch in range(self.epochs):
            ...
            for i, batch in pbar:
                ...
                X_hat, loss_latent = self.model(X)
                loss_recon = nn.functional.mse_loss(X, X_hat, reduction="mean")
                loss = self.recon_weight * loss_recon + self.latent_weight * loss_latent
